	...
	int query_up(int bajo, int alto) {
		...
		while (true) {
			...
			if (bChain == aChain) {
				...
				ans = mxx(ans, st->query(posInSeg[alto] + 1, posInSeg[bajo]));
				break;
			}
 
			ans = mxx(ans, st->query(posInSeg[ head[ bChain ] ], posInSeg[bajo]));
			bajo = head[ bChain ];
			bajo = papa[bajo][0];
		}
